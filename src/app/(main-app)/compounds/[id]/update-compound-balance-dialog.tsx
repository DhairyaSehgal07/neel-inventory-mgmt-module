...
type UpdateCompoundBalanceDialogProps = {
  compoundId: number;
  currentBalance: number;
};

export function UpdateCompoundBalanceDialog({
  compoundId,
  currentBalance,
}: UpdateCompoundBalanceDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/compounds/${compoundId}/update-compound-quantity`, {
        method: 'POST',
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <FieldGroup>
              <Field>
                <FieldLabel>Enter closing balance (kg)</FieldLabel>
                <Input
                  type="number"
                  min={0}
                  step="any"
                  placeholder={`Current: ${currentBalance} kg`}
